...
function list(req, res, next) {
	const {
    limit = 50, skip = 0, q, functions = ['clc', 'trc', 'tl', 'tb'], deleted = false
  } = req.query;

	const query = Object.assign({ deleted }, q ? {
		$or: [{
			username: {
				$regex: q,
				$options: "i"
	...
	Account.list(query, {
		limit,
		skip
	}).then(accounts => res.json(accounts)).catch(e => next(e));
}
